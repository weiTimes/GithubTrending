...
export default class SortKeyPage extends Component {
    ...
    _getSortResult() {
        ...
        // 遍历已经订阅的排序前的数组
        for (let i = 0, len = this.originalCheckArray.length; i < len; i++) {
            let item = this.originalCheckArray[i];
            let index = this.dataArray.indexOf(item); // 获得索引
            this.sortResultArray.splice(index, 1, this.state.checkedArray[i]); // 在相应的索引位置插入排序后的已订阅标签数据
        }
    }
     ...
     */
    _onSave(isChecked) {
        // 如果没有检查顺序是否改变，则需要进行检查
        // 如果两个数组相等，那么用户没有进行拖拽操作，也就是顺序没有发生改变
        if (!isChecked && ArrayUtils.isEqual(this.state.checkedArray, this.originalCheckArray)) {
            this.props.navigator.pop();
            return;
        }
        // 顺序发生改变执行以下操作
